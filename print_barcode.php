...
if (!isset($_GET['id']) || !isset($_GET['prints'])) {
    die("Product ID or print count not provided.");
}

// Get the list of product IDs from the query string
$productIds = explode(',', $_GET['id']);  // Assuming product IDs are comma-separated
$prints = intval($_GET['prints']); // Get the print count

// Iterate through each product ID
foreach ($productIds as $productId) {
    $productId = intval($productId);  // Ensure it's an integer
    ...
    if ($result->num_rows > 0) {
        ...
        // Print Barcode for the current product
        for ($i = 0; $i < $prints; $i++) {
            ?>
        ...
        die("Product not found for ID: $productId");
    }
}
?>
